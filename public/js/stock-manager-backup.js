...
async function createDefaultFormats() {
  ...
  try {
    ...
    if (createdFormats.length > 0) {
      ...
      if (formatSelect && formats && formats.length > 0) {
        formatSelect.innerHTML = '';
        formats.forEach(format => {
          const option = document.createElement('option');
          option.value = format.id;
      ...
      if (!formatId) {
        ...
        try {
          ...
          if (newFormatResponse && newFormatResponse.success) {
            ...
            console.log('Échec de la création directe du format, tentative alternative');
            ...
            if (altFormatResponse && altFormatResponse.success) {
              ...
              if (updateResponse && updateResponse.success) {
                ...
                console.error('Échec de l\'association du format au produit');
                // Continuer quand même, car le format existe, mais n'est pas associé
                formatId = altFormatId;
              }
            } else {
              console.error('Échec de la création du format (méthode alternative)');
            }
          }
        } catch (formatError) {
...
async function tryUpdateStock(endpoint, stockId, newQuantity, reason) {
  try {
    ...
    if (success) {
      ...
      console.error('*** DEBUG: Échec de la mise à jour:', result);
      return false;
    }
  } catch (error) {
...
// Appeler la fonction d'urgence au chargement du script
/*
(async function() {
  try {
    await emergencyFormatFix();
  } catch (error) {
    console.error('Échec de la réparation d\'urgence des formats:', error);
  }
})();
*/
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Écouteur d'événement pour le sélecteur de bar
...
async function saveAllStocks(barId) {
  try {
    ...
    if (!result.success) {
      console.log("Échec de l'initialisation groupée, tentative d'initialisation individuelle");
      ...
      for (const stock of stocks) {
        try {
          ...
          if (stockResult && stockResult.success) {
            ...
            console.error(`Échec de l'ajout du stock pour format ${stock.formatId}:`, stockResult);
            errorCount++;
          }
        } catch (e) {
      ...
      if (successCount > 0) {
        ...
        showAlert(`Échec de l'initialisation de tous les stocks`, 'danger');
        return false;
      }
    }
...
async function loadRestockSummary(barId) {
  ...
  if (!restockSummaryContent) {
    console.error('Élément restock-summary-content non trouvé');
    return;
  }
  ...
  try {
    ...
    if (!barResponse || !barResponse.success) {
      console.error('Échec de récupération des informations du bar');
    ...
    if (!stocksResponse || !stocksResponse.success) {
      console.error('Échec de récupération des stocks');
...
// SUPPRIMÉ: Deuxième DOMContentLoaded dupliqué pour éviter les conflits
// La logique de gestion des événements est déjà dans le premier DOMContentLoaded (ligne 816)
